...
import connectDotsSrc from '../connectDotsRenderer.js?raw';
...
<head>
<meta charset="UTF-8">
<title>${config.pageTitle || 'Соедини точки'}</title>
...
document.fonts.ready.then(async () => {
  ...
  renderConnectDotsAllPages({...${JSON.stringify(config)}, sprunkiImages});
});
<\/script>
</body>
</html>`;
...
async function doPDF(config, setStatus) {
  ...
  try {
    ...
    for (let i = 0; i < (config.pages || 1); i++) {
      ...
      renderConnectDotsPage(canvas.getContext('2d'), { ...config, sprunkiImages }, i);
      if (i > 0) pdf.addPage();
      pdf.addImage(canvas.toDataURL('image/jpeg', 0.92), 'JPEG', 0, 0, A4_W, A4_H);
    }
    pdf.save(`connect-dots.pdf`);
  } finally {
    setStatus('idle');
  }
}
...
async function doSave(config) {
  ...
  a.download = `connect-dots.html`;
  a.click();
  URL.revokeObjectURL(a.href);
}

export default function ConnectDotsExportButtons({ config, setConfig }) {
